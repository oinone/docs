...
export default defineClientConfig({
  enhance({app, router, siteData}) {
    ...
    router.beforeEach((to, from, next) => {
      if (!__VUEPRESS_SSR__) {
        if (!to.fullPath.startsWith('/handbook')) {
          if (to.fullPath.startsWith('http') || to.fullPath.startsWith('https')) {
            window.location.href = to.fullPath
          } else if (to.fullPath === '' || to.fullPath === '/') {
            window.location.href = WELCOME_WEBSITE_URL + '/document'
          } else if (to.fullPath === '/$welcomeHomePage') {
            window.location.href = WELCOME_WEBSITE_URL
          } else {
            window.location.href = WELCOME_WEBSITE_URL + (to.fullPath.startsWith('/') ? '' : '/') + to.fullPath
          }
        } else {
          if (to.fullPath === '/handbook' || to.fullPath === '/handbook/') {
            window.location.href = WELCOME_WEBSITE_URL + '/document'
          } else {
            next()
          }
